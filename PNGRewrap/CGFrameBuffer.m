...
- (NSData*) formatAsPNG
{
  ...
  @autoreleasepool {
    ...
    dataDest = CGImageDestinationCreateWithData((__bridge CFMutableDataRef)mData,
                                                type,
                                                count,
                                                NULL);
    assert(dataDest);
...
// Convert pixels to a BMP image format that can be easily saved to disk.
...
- (NSData*) formatAsTIFF
{
  ...
  @autoreleasepool {
    ...
    // Render buffer as a TIFF image with LZW compression
    ...
    const uint32_t my_NSTIFFCompressionLZW = 5;
    
    NSDictionary *propsDict = @{ (NSString*)kCGImagePropertyTIFFCompression : @(my_NSTIFFCompressionLZW) };
    
    NSDictionary *tiffDict = @{ (NSString*)kCGImagePropertyTIFFDictionary : propsDict };
    
    CGImageDestinationAddImage(dataDest, imgRef, (__bridge CFDictionaryRef) tiffDict);
    CGImageDestinationFinalize(dataDest);
